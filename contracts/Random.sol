...
contract ChainlinkRandoms is AccessControl, IRandoms, VRFConsumerBase {
  bytes32 public constant RANDOMNESS_REQUESTER =
    keccak256("RANDOMNESS_REQUESTER");
    ...
    returns (uint256)
  {
    return
      uint256(
        keccak256(
          abi.encodePacked(user, seed, block.timestamp, block.difficulty)
        )
      );
  }
  ...
  function requestRandomNumber() external {
    require(
      hasRole(RANDOMNESS_REQUESTER, msg.sender),
      "Sender cannot request seed"
    );
